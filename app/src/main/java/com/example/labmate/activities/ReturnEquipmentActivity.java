...
public class ReturnEquipmentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        equipmentQR = findViewById(R.id.actEquipmentQR);
        ...
        if (!userSession.canManageInventory()) {
            ...
            Toast.makeText(this, "You do not have permission to return equipment.", Toast.LENGTH_LONG).show();
    ...
    private void returnEquipment(String qrId) {
        ...
        equipmentRepository.findByQrId(
                ...
                snapshot -> {
                    ...
                    DocumentSnapshot equipmentDoc = snapshot.getDocuments().get(0);

                    String equipmentId = equipmentDoc.getId();

                    String state = equipmentDoc.getString("state");

                    if (!Constants.STATE_BORROWED.equals(state)) {

                        Toast.makeText(this, "This equipment is not currently borrowed.", Toast.LENGTH_LONG).show();

                        return;
                    }

                    findBorrowing(equipmentId);
                },
                e -> Toast.makeText(
                        this,
                        e.getMessage() != null ? e.getMessage() : "Failed to find equipment.",
                        Toast.LENGTH_LONG
                ).show()
        );
    }
    ...
    private void findBorrowing(String equipmentId) {
        ...
        borrowingRepository.getBorrowedBorrowingByEquipmentId(
                ...
                snapshot -> {
                    ...
                    DocumentSnapshot borrowingDoc = snapshot.getDocuments().get(0);

                    String borrowingId = borrowingDoc.getId();

                    completeReturn(equipmentId, borrowingId);
                },
                e -> Toast.makeText(
                        this,
                        e.getMessage() != null ? e.getMessage() : "Failed to find borrowing.",
                        Toast.LENGTH_LONG
                ).show()
        );
    }
    ...
    private void completeReturn(String equipmentId, String borrowingId) {

        WriteBatch batch = FirebaseFirestore
                .getInstance()
                .batch();

        DocumentReference borrowingRef = FirebaseFirestore
                .getInstance()
                .collection("borrowings")
                .document(borrowingId);

        DocumentReference equipmentRef = FirebaseFirestore
                .getInstance()
        ...
        batch.commit()
                .addOnSuccessListener(unused -> {
                    ...
                    Toast.makeText(this, "Equipment Successfully Returned", Toast.LENGTH_LONG).show();
                ...
                .addOnFailureListener(e -> {
                    ...
                    Toast.makeText(
                            this,
                            e.getMessage() != null ? e.getMessage() : "Failed to return equipment.",
                            Toast.LENGTH_LONG
                    ).show();
                });
    }
